...
while true; do
	...
	if [ -f "$TOKEN_FILE" ]; then
		...
		if (($(echo "$PERCENT >= 60" | bc -l))) && [ "$NOTIFIED_60" = false ]; then
			...
			nancy direct "$TASK_ID" "CRITICAL: Token budget at ${PERCENT}%. You can leave uncommitted work the next agent will pick up from were you left off." --type directive
			NOTIFIED_60=true
